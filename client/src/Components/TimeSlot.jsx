...
class TimeSlot extends React.Component {
  ...
  goToHere(currMinutes) {
    ...
    newTimeSlot.startTime = new Date(
      this.state.stub + this.state.startTime * 60 * 1000
    );
    newTimeSlot.endTime = new Date(
      this.state.stub + (currMinutes + 15) * 60 * 1000
    );
  ...
  render() {
    ...
    let blocks = [];
    let todayStart =
      this.props.timeSlot.startTime.getHours() * 60 +
      this.props.timeSlot.startTime.getMinutes();
    let todayEnd =
      this.props.timeSlot.endTime.getHours() * 60 +
      this.props.timeSlot.endTime.getMinutes();
