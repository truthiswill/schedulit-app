...
class ChooseHours extends React.Component {
  ...
  hoursToString(time) {
    ...
    return `${String(hour).padStart(2, '0')}:${String(minutes).padStart(2, '0')} ${isBeforeNoon ? 'AM' : 'PM'}`
  }
  handleChange(e) {
  ...
  addTimesToSet() {
    this.props.addTimesToSet({ startTime, endTime })
  }
      ...
      return (
        <div>
          <select id="startTime" onChange={this.handleChange}>
            {times.map((time) => <option value={time}>{this.hoursToString(time)} </option>)}
          </select>
          <select id="endTime" onChange={this.handleChange}
